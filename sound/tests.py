...
def Make_User(self, year=2019):
    user = User.objects.create_user(email=str(year) + "mail@gmail.com", year=year)
...
def Make_Song(self, user_year=2019):
    self.live = Live.objects.create(
        live_name="Test Rehasal 1", updated_by=User.objects.get(email=str(user_year) + "mail@gmail.com")
    )
...
class uploadTest(TestCase):
    ...
    def test_sound_upload_logIN_with_permission(self):
        ...
        with open(mp3dir, "rb") as file:
            ...
            response = self.client.get("/sound/" + str(live.pk) + "/json/")
            self.assertEqual(response.status_code, 200)
