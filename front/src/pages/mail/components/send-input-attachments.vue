<template>
  <div class="attachent-wrapper form-one-row">
    <label
...
export default {
  ...
  setup() {
    ...
    const error_messages = computed(
      () => store.state.send.attachments["error_messages"]
    );
    ...
    const commit = () => {
      ...
      const uniques = internalAttachments.value.reduce((prev, current) => {
        if (
          !prev.some((f) => f.size === current.size && f.name === current.name)
        ) {
          return [...prev, current];
        } else {
          // 重複あり
      ...
      uniques.forEach((f) => {
        internalAttachments.value.push(f);
      });
