...
class MailStatusApiTest(TestCase):
    def setUp(self):
        ...
        self.api_endpoint = "http://testserver/mail/status_api"
    ...
    def test_get_without_auth(self):
        ...
        response = self.client.get("/mail/status_api?message_id=12345")
        self.assertEqual(response.status_code, 403)

    def test_get_without_message_id(self):
        # message_id パラメーターがない場合、BadRequest が返る
        response = self.client.get("/mail/status_api", HTTP_X_AUTHORIZATION="testkey")
        self.assertEqual(response.status_code, 400)

    def test_get_non_existent_mail(self):
        # 存在しない message_id の場合、BadRequest が返る
        response = self.client.get(
            "/mail/status_api?message_id=nonexistent", HTTP_X_AUTHORIZATION="testkey"
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_get_success(self):
        ...
        response = self.client.get(
            "/mail/status_api?message_id=12345", HTTP_X_AUTHORIZATION="testkey"
        )
    ...
    def test_post_without_auth(self):
        # 認証ヘッダーがない場合、Forbidden が返る
        response = self.client.post(
            "/mail/status_api", {"message_id": "12345", "status": "sent"}
        )
        self.assertEqual(response.status_code, 403)
