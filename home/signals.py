...
def update_send_mail_address(sender, instance, **kwargs):
    kansou_list = [
        {"genre": "感想用紙", "title": q.title, "date": q.created_at, "path": "/kansou/"}
        for q in Kansouyoushi.objects.order_by("-created_at")[:LIST_NUM]
    ]
    movie_list = [
        {"genre": "ライブ映像", "title": q.title, "date": q.created_at, "path": "/movie/"}
        for q in YoutubeURL.objects.order_by("-created_at")[:LIST_NUM]
    ]
    pictures_list = [
        {"genre": "写真", "title": q.title, "date": q.created_at, "path": "/photo/"}
        for q in Album.objects.order_by("-created_at")[:LIST_NUM]
    ]
    sound_list = [
        {"genre": "リハ音源", "title": q.live_name, "date": q.created_at, "path": "/sound/{}/".format(q.pk)}
