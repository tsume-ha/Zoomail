...
def ContentLog(LIST_NUM=3):
    ...
    kansou_list = [('kansou', '感想用紙', q.created_at, q.title, reverse('kansou:index')) for q in Kansouyoushi.objects.order_by('-created_at')[:LIST_NUM]]
    ...
    sound_list = [('sound', 'リハ音源', q.updated_at, q.live_name, reverse('sound:playlist', args=[q.pk])) for q in Live.objects.order_by('-updated_at')[:LIST_NUM]]
