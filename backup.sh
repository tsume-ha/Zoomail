...
# dump the database
docker compose -f compose.prod.yaml exec database mariadb-dump --all-databases -uroot -p"$DATABASE_PASSWORD" > /zoomail_db_dump/zoomail_$now.sql

# compress the database dump
tar -czvf /zoomail_db_dump/zoomail_$now.tar.gz /zoomail_db_dump/zoomail_$now.sql

# remove the uncompressed database dump
rm /zoomail_db_dump/zoomail_$now.sql
